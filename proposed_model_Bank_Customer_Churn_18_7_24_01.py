...
# Updated load_clean_data function
def load_clean_data(filename):
    ...
    return X, y, trainX, testX, trainy, testy, valX, valy, shapes_info
...
def main():
    ...
    experiment_folder = '/data2/cmdir/home/hieuvv/CIT/Bank_Customer_Churn'
    model_name = "proposed_model_Bank_Customer_Churn_18_7_24_01"
    ...
    X, y, trainX, testX, trainy, testy, valX, valy, shapes_info = load_clean_data(data_path)
    ...
    models = {
        'LightGBM': lgbm.LGBMClassifier(n_estimators=1500, random_state=42),
    ...
    ensemble_folder = create_model_folder(result_folder, ensemble_name)
    ...
    logging.debug("Training ensemble model: VotingClassifier")
    voting_clf.fit(trainX, trainy)
    evaluate_model(voting_clf, testX, testy, ensemble_folder, ensemble_name, metrics_dict)
    cross_val_metrics(voting_clf, X, y, metrics_dict, ensemble_name)
    save_model(voting_clf, ensemble_name, result_folder)

    model_names = ['LightGBM', 'XGBoost', 'Bagging', 'AdaBoost', 'Ensemble_Voting']

    members = load_all_models(model_names, result_folder)
    
    stacked_model = fit_stacked_model(members, trainX, trainy)
    
    stacked_model_name = 'Stacking_Model'
    ...
    save_model(stacked_model, stacked_model_name, result_folder)

    save_metrics_to_csv(list(metrics_dict.values()), os.path.join(result_folder, 'final_metrics.csv'))
